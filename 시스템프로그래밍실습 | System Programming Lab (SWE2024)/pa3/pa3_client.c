...
void parse_request(char *request_line, Request *request);
...
int main(int argc, char *argv[]) {
  ...
  // method 1: file input
  if (argc < 4) // if no file path is provided, go to REPL
    goto REPL;
  ...
  char *file_line = NULL;
  size_t len = 0;
  // read file line by line for commands
  while (getline(&file_line, &len, cmd_file) != -1) {
    ...
    // parse request
    Request request;
    parse_request(file_line, &request);

    // send request to server
    handle_action(&request);
  }

  // send termination request (0 0 0)
  Request termination_request;
  termination_request.user = 0;
  termination_request.action = TERMINATION;

  uint8_t termination_data = 0; // data field is 0
  termination_request.size = sizeof(termination_data);
  termination_request.data = &termination_data;

  handle_action(&termination_request);
  goto end; // terminate the program

  // method 2: REPL =================================================================
  // infinite loop until it has to terminate
REPL:
  char *request_line;
  while ((request_line = readline("Enter request >> "))) {
    ...
    // parse request
    Request request;
    parse_request(request_line, &request);
...
void handle_action(Request *request) {
  ...
  case TERMINATION:
    // send the termination request (logout is taken care of in the server)
  ...
  case CONFIRM_BOOKING:
    ...
    if (recv_response(conn_fd, &response) == 0) {
      if (response.code == 0) { // successful confirmation
        ...
        if (convert_data_to_int(request->data, request->size) == 0) {
          if (response.size == 0)
            ...
            printf("Available seats: %s\n", convert_data_to_string(response.data, response.size));
        }
        // if the data field is empty, return the booked seats by the user
        else if (request->data == NULL && request->size == 0) {
          if (response.size == 0)
...
void print_error_msg(Request *request, Response *response) {
  ...
  case LOGIN:
    if (response->code == 1)
      ...
      printf("Failed to login as user id is out of range\n");
    break;
  ...
  case CONFIRM_BOOKING:
    if (response->code == 1)
      ...
      printf("Failed to confirm booking as arguments are invalid\n");
    break;
...
// HELPER FUNCTIONS ============================================================
// parses the request line and fills the request struct
void parse_request(char *request_line, Request *request) {
  char *endptr;
  long long user_id = strtoll(request_line, &endptr, 10);
  uint8_t action = (uint8_t)strtoul(endptr, &endptr, 10);

  // check if user id is out of range, if not set it to 0
  // do not set user id to 0 if the action is TERMINATION
  if((user_id < 1 || user_id > UINT32_MAX) && action != TERMINATION) 
    request->user = 0;
  else
    request->user = (uint32_t)user_id;
