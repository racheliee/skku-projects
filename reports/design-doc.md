...
header-includes: |
  ...
  \usepackage{fancyhdr}
  \pagestyle{fancy}
  \fancyhead[L]{CSE 113 Homework 1 Design Document}
  \fancyhead[C]{}
  \fancyhead[R]{\thepage}
  \fancyfoot{}
  \usepackage{float}
...
abstract: |
  This is the design document for the first homework assignment of CSE113: Parallel and Concurrent Programming.
---
...
# Part 1: Loop Unrolling Independent Iterations for ILP

## Intuition

The loop structure provided in the assignment is chracterized by a dependency chain of length `N` (64 in this specific case). Since the dependency chain cannot be re-ordered or pre-computed due to the non-associative nature of floating point addition, the loop iterations are dependent on each other.
...
Since the chain length is fixed at 64, the loop is unrolled by a factor of 64. (i.e. $k = 1, 2, 4, 8, ... 64$). Hence, no cleanup iterations are needed

## Approach

The roop will be unrolled in two different ways: sequential loop unrolling and interleaved loop unrolling.
...
Since the size and unroll factor is always a power of 2, no cleanup iterations are needed.

## Approach to reduction loops
...
# Part 3: SPMD Parallel Programming Using C++ Threads
