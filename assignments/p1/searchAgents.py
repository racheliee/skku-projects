...
def foodHeuristic(state, problem):
    ...
    # *** Your Code Here ***
    # print(f"food list: {state[1].asList()}")
    # print(f"state: {state}")
    # print(f"problem walls: {problem.walls}")
    ''' problem walls:
    TTTTT
    TFFFT
    TTTFT
    ...
    #     distances.append(manhattanDistance(currPos, food))
    # return max(distances) if len(distances) > 0 else 0

    # use greedy algorithm => failed admissibility test
    # heuristic = 0

    # while (len(foodlist)):
    #     distances = []
    #     for food in foodlist:
    #         distances.append((maze(currPos, food, problem.startingGameState), food))
    #     minDistance, closestFood = min(distances)
    #     heuristic += minDistance
    #     currPos = closestFood
    #     foodlist.remove(closestFood)

    # find the path to the food within the maze (account walls)
    distance = []
    for food in foodlist:
        distance.append(maze(currPos, food, problem.startingGameState))

    return max(distance) if len(distance) > 0 else 0
    # return heuristic.null(state, problem)  # Default to the null heuristic.


def not_oob(x, y, rows, cols):
